        ...
        <div className="grid md:grid-cols-3 gap-8">
          {values.map((v, i) => (
            <div key={v.title} className="relative p-8 bg-card rounded-2xl border border-border">
              <div className="absolute -top-4 left-8 h-10 w-10 rounded-full bg-gradient-gold flex items-center justify-center font-serif text-lg text-foreground shadow-gold">
                {i + 1}
              </div>
              <h3 className="font-serif text-2xl text-foreground mb-4 mt-3">{v.title}</h3>
